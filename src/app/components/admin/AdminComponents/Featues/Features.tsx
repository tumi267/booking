...
function FeaturesSection({location,sectionNum,viewport}:{location: string;
  sectionNum: string;
  viewport:'desktop' | 'tablet' | 'mobile'}) {
 const {features, current, setSelectedId,showEditor,setShowEditor,isLoading,sectionStyle, addFeature,handleSave,selected,updateFeature,removeFeature,toggleSection,openSections,updateBreakpoint}=useAdminFeat(location,sectionNum,viewport)
  ...
  return (
    <div style={current.section}>
      ...
      <button onClick={addFeature}>Add Feature</button>
          ...
          {openSections.section && (<><label style={labelStyle}>Margin Top</label>
          <input
            style={inputStyle}
            type="number"
            value={parseInt(current.section.marginTop as string) || 40}
            // onChange={(e) =>
          ...
          <input
            style={inputStyle}
            type="number"
            value={parseInt(current.section.marginBottom as string) || 40}
            // onChange={(e) =>
          ...
          <input
            ...
            onChange={(e) =>updateBreakpoint('grid', {columns: Number(e.target.value),})
            }
          />
          ...
          <input
            style={inputStyle}
            type="color"
            value={current.card.background as string}
